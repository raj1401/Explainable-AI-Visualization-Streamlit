...
if 'dates_series' not in sl.session_state:
    sl.session_state.dates_series = None
    ...
    if model_type == "Random Forest Classifier":
        ...
        with model_col:
            with sl.spinner("Searching for Optimum Parameters"):
                params_, model_, df_, dates_ = create_and_search_tree_classifier(data_file=data, param_distributions=param_distributions)
        sl.session_state.search_results["params"] = params_
        sl.session_state.search_results["model"] = model_
        sl.session_state.dataframe = df_
        sl.session_state.dates_series = dates_
        ...
        with model_col:
            with sl.spinner("Searching for Optimum Parameters:"):
                params_, model_, df_, dates_ = create_and_search_tree_regressor(data_file=data, param_distributions=param_distributions)
        sl.session_state.search_results["params"] = params_
        sl.session_state.search_results["model"] = model_
        sl.session_state.dataframe = df_
        sl.session_state.dates_series = dates_        
...
def plot_regression_time_series_predictions(df, model, col):
    fig, err_msg = get_regression_time_series_predictions(df=df, model=model,
                                               random_state=sl.session_state.TRAIN_TEST_RANDOM_STATE,
                                               test_fraction=sl.session_state.TEST_FRACTION,
                                               dates = sl.session_state.dates_series)
    if fig is None:
        col.write(err_msg)
    else:
        col.write(fig)
...
# ---- PAGE CONFIG ---- #
sl.set_page_config(page_title="Visualize SHAP Values", layout='wide')

# ---- HEADER SECTION ---- #
sl.markdown("<h1 style='text-align: center;'> Visualize SHAP Values </h1>", unsafe_allow_html=True)
sl.markdown("<h5 style='text-align: center;'> Made By Raj Mehta </h5>", unsafe_allow_html=True)

sl.write('---')

sl.markdown("""
            This app allows you to visualize SHAP values of a model's performance
            on the selected data. If the data has *n* columns then the first column 
            should correspond to the time the data was recorded, the next 
            *n-1* columns correspond to input feature data and the last column 
            corresponds to the target labels.
""")
...
with sl.container():
    if "params" in sl.session_state.search_results:
        sl.write("""
                ...
                using five-fold cross validation. Do you want to train 
                with these parameters? You can set them yourself too.
        """)
...
with sl.container():
    if sl.session_state.model_on_selected_feats is not None and len(new_features) != 0:
        # Model Performance
